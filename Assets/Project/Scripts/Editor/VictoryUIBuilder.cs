...
public static class VictoryUIBuilder
{
    ...
    private static readonly Color Gold = new Color(0.949f, 0.780f, 0.349f, 1f);

    [MenuItem("Tools/Augmentra/Build Victory UI")]
    public static void Build()
    {
        ...
        if (font == null)
        {
            EditorUtility.DisplayDialog("Build Victory UI", "Could not find the Nexa Extra font asset at:\n" + FontPath, "OK");
            return;
        }

        Sprite uiSprite = AssetDatabase.GetBuiltinExtraResource<Sprite>("UI/Skin/UISprite.psd");

        GameObject oldCanvas = GameObject.Find("VictoryCanvas");
        if (oldCanvas != null)
        {
            Object.DestroyImmediate(oldCanvas);
        }

        GameObject oldManager = GameObject.Find("VictoryUI");
        if (oldManager != null)
        {
        ...
        GameObject canvasGo = new GameObject("VictoryCanvas", typeof(RectTransform), typeof(Canvas), typeof(CanvasScaler), typeof(GraphicRaycaster));
        ...
        GameObject managerGo = new GameObject("VictoryUI");
        VictoryUI victoryUI = managerGo.AddComponent<VictoryUI>();
        ...
        TextMeshProUGUI title = AddText("Title", panel.transform, font, "VICTORY", 64f, FontStyles.Bold, Gold);
        ...
        victoryUI.group = panelGroup;
        victoryUI.statsText = stats;
        victoryUI.restartButton = restartButton;
        victoryUI.mainMenuButton = mainMenuButton;
        ...
        {
            EditorUtility.DisplayDialog("Build Victory UI",
                "Victory UI built and wired.\n\nVictoryUI added with panel, stats text and Restart/Main Menu buttons assigned.\n\nSave the scene (Ctrl+S) to keep it.",
                "OK");
        }
    }
